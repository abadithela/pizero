        ...
        for sim_data_num in "${sim_data_nums[@]}"; do
            for real_data_num in "${real_data_nums[@]}"; do
                ...
                job_id2=$(sbatch --parsable --dependency=afterok:$job_id compute_norm_stats.sh \
                 --repo_id ${additional_name}_${path_group}_seed${seed}_sim${sim_data_num}_real$((num_base_demos + real_data_num))_pi
                 )

                
                sbatch --dependency=afterok:$job_id2 train.sh \
                    --exp-name ${additional_name}_${path_group}_seed${seed}_sim${sim_data_num}_real$((num_base_demos + real_data_num))_pi \
                    --repo_id ${additional_name}_${path_group}_seed${seed}_sim${sim_data_num}_real$((num_base_demos + real_data_num))_pi
